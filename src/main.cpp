...
#include <glad/gl.h> // Initialize with gladLoaderLoadGL()
...
int main(int, char **)
{
...
#if defined(IMGUI_IMPL_OPENGL_LOADER_GL3W)
    int version = gl3wInit() != 0;
#elif defined(IMGUI_IMPL_OPENGL_LOADER_GLEW)
    int version = glewInit() != GLEW_OK;
#elif defined(IMGUI_IMPL_OPENGL_LOADER_GLAD)
...
#else
    int version = 0; // If you use IMGUI_IMPL_OPENGL_LOADER_CUSTOM, your loader is likely to requires some form of initialization.
#endif
    ...
    int image_width = 0;
    int image_height = 0;
    unsigned char *image_data = stbi_load("assets/icon.png", &image_width, &image_height, NULL, 4);
    if (image_data == NULL)
    {
    ...
    printf("OpenGL texture filtering parameters set\n");
